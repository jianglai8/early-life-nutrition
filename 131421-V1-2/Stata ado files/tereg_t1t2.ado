...
cap program drop tereg_t1t2;
	
program define tereg_t1t2, rclass;
	...
	*******************************************************************************/
	...
	if (`numml' > 0 & `numel'>0) {;
		...
		/* SEPARATE EFFECTS ------------------------------- */
		reg `outc' 	i.treat##i.wave `controls' if `tousef', `options';
		...
		return local p12ml = (strtrim("`: di %10.2f r(p)'"));
		
		/* EL effects */
		lincom 1.treat + 1.treat#3.wave;
		local b = r(estimate); local s = r(se); prncoef, coef(`b') serr(`s') text par;
		return local bt1el = r(betaout); return local st1el = r(seout);
		lincom 2.treat + 2.treat#3.wave;
		local b = r(estimate); local s = r(se); prncoef, coef(`b') serr(`s') text par;
		return local bt2el = r(betaout); return local st2el = r(seout);
		test 1.treat + 1.treat#3.wave = 2.treat + 2.treat#3.wave;
		return local p12el = (strtrim("`: di %10.2f r(p)'"));
	
		
		/* POOLED EFFECTS --------------------------------- */
		reg `outc' 	i.treatp##i.wave `controls' if `tousef', `options';
		...
		return local pmlel = (strtrim("`: di %10.2f r(p)'"));
